...
export class UserComponent extends BaseComponent implements OnInit {
  ...
  tableConfig: ITableConfig[] = TABLE_CONFIG.TABLE_ADMIN_USER;
  isLoading = true;
  ...
  getCustomers(params: BaseQueryRequest = {}) {

    this.subscribeOnce(this.firebaseService.getTableData('users', params.page || 1 , params.size || 25), {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      onSuccess: (data : any) => {
        this.activeUser = (data.records as []).reduce((acc, record) => {
          if (record['isActive']) { // Kiểm tra nếu `isActive` là false
            return acc + 1; 
          }
          return acc;
        }, 0); 
