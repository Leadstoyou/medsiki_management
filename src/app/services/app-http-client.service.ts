...
export class AppHttpClient {
  ...
  protected clientTimezone: string;
  ...
  public exportRecord(uri: string, params = {}): Observable<HttpResponse<Blob>> {
    const httpParams = this.generateHttpParams(params);
    return this.httpClient
      .get<Blob>(this.prefixUri(uri), {
        observe: 'response',
        responseType: 'blob' as 'json',
        params: httpParams,
        headers: this.generateHttpHeaders(),
  ...
  public get<T>(uri: string, params = {}, configs: any = {}): Observable<BaseResponseApi<T>> {
    const httpParams = this.generateHttpParams(params);
    return this.httpClient
      .get<BaseResponseApi<T>>(this.prefixUri(uri), {
        reportProgress: configs.reportProgress,
        params: httpParams,
        headers: this.generateHttpHeaders({ ...configs.headers, timezone: this.clientTimezone }),
      })
      .pipe(catchError((err) => this.handleError(err)));
  }
  ...
  public put<T>(uri: string, params: object = {}): Observable<BaseResponseApi<T>> {
    return this.httpClient
  ...
  public patch<T>(uri: string, params: object = {}, configs: any = {}): Observable<BaseResponseApi<T>> {
    return this.httpClient
  ...
  public delete<T>(uri: string, body: any = {}): Observable<BaseResponseApi<T>> {
    return this.httpClient
  ...
  private generateHttpHeaders(headerInfo?: { [key: string]: any }, isMultiPart = false) {
